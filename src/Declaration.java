public class Declaration implements Constante {
	...
	public void createConstInt(int v) {
		if (Yaka.tabIdent.existeIdent(this.lastIdent)) {
			System.out.println("Erreur : l'ident '" + this.lastIdent + "' existe déjà (ligne : " + Yaka.token.beginLine + ").");
		}
		
		Yaka.tabIdent.rangeIdent(this.lastIdent, new IdConst(this.lastIdent, eType.ENTIER, v));
	}
	...
	public void createConstBool(int b) {
		...
		Yaka.tabIdent.rangeIdent(this.lastIdent, new IdConst(this.lastIdent, eType.BOOLEEN, b));
	}
	...
	public void createVar(String name) {
		if (Yaka.tabIdent.existeIdent(name)) {
			System.out.println("Erreur: l'ident '" + name + "' existe deja (ligne:" + Yaka.token.beginLine + ").");
		}
		
		this.offset -= 2;
		Yaka.tabIdent.rangeIdent(name, new IdVar(name, this.lastType, this.offset));
	}
